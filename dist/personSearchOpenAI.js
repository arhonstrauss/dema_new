...
export async function searchPersonOpenAI(options) {
    const { name, address, context, fromDate, toDate, maxResults, stream = false, includeInstagram = false, includeFacebook = false, includeNews = true } = options;
  ...
  `;
    ...
    if (stream) {
        let usage = undefined;
        const result = await openaiCompletionStream(requestBody, (token) => {
            console.log(token);
        });
        return {
            usage: result.usage
...
//# sourceMappingURL=personSearchOpenAI.js.map