...
type Run = (p: any) => Promise<any>;

export const routes: Record<string, Run> = {
  // Twitter/X
...
export async function runIntent(intent: string, params: any) {
  const fn = routes[intent];
  if (!fn) throw new Error(`Unknown intent: ${intent}`);
  return fn(params || {});
}

function assertStr(v: any, name: string) {
