...
export async function synthesizeToJSON(rawOutput: string): Promise<PoliticalAnalysis> {
...
- Only output valid json, do not wrap in md code fences or anything else`;
  ...
  try {
    ...
    const analysis: PoliticalAnalysis = JSON.parse(jsonText);
    ...
    (analysis as any).fullOutput = rawOutput;
    return analysis;
  } catch (error) {
